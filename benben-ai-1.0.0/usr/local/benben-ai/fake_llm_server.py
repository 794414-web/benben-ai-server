...
class RequestDeduplicator:
    """请求去重器 - 防止奔奔助手重复发送相同请求"""
    
    def __init__(self, max_requests=200, dedup_window=30):
        # 记录最近处理的请求: {request_hash: timestamp}
        self._recent_requests = OrderedDict()
        # 记录最近执行的命令: {command_hash: timestamp}
        self._recent_commands = OrderedDict()
        # 待确认的命令: {session_key: {timestamp, tool_calls, confirmed}}
        self._pending_commands = {}
        self._max_requests = max_requests
        self._dedup_window = dedup_window  # 去重窗口（秒）
    ...
    def _cleanup_expired(self):
        ...
        # 清理过期的请求记录
        expired_keys = [k for k, v in self._recent_requests.items() 
                      if now - v > self._dedup_window]
        for k in expired_keys:
            del self._recent_requests[k]
        
        # 清理过期的命令记录
        expired_keys = [k for k, v in self._recent_commands.items() 
                      if now - v > self._dedup_window]
        for k in expired_keys:
            del self._recent_commands[k]
    
    def _get_request_hash(self, user_text, tool_calls=None):
        """生成请求的唯一哈希"""
        ...
        if tool_calls:
            ...
            content = f"{text}|{calls_str}"
        else:
            content = text
        return hashlib.md5(content.encode('utf-8')).hexdigest()
    
    def _get_session_key(self, request_body):
        """获取会话标识"""
        # 优先级: user字段 > system消息 > 默认
        user = request_body.get("user", "")
        if user:
            return f"user_{user}"
        
        # 从Authorization header或其他字段
        headers = request_body.get("_headers", {})
        if headers:
            auth = headers.get("authorization", "") or headers.get("Authorization", "")
            if auth:
                return f"auth_{hashlib.md5(auth.encode()).hexdigest()[:8]}"
        
        return "anonymous"
    
    def check_and_record_request(self, user_text, tool_calls=None):
        """
        检查请求是否重复，如果是则返回True
        同时记录请求
        """
        ...
        request_hash = self._get_request_hash(user_text, tool_calls)
        now = time.time()
        
        # 检查是否为重复请求
        if request_hash in self._recent_requests:
            last_time = self._recent_requests[request_hash]
            if now - last_time < self._dedup_window:
                return True  # 是重复请求
        
        # 记录请求
        self._recent_requests[request_hash] = now
        
        # 限制大小
        if len(self._recent_requests) > self._max_requests:
    ...
    def record_command(self, request_body, tool_calls):
        ...
        now = time.time()
        session_key = self._get_session_key(request_body)
        
        # 记录到待确认命令
        self._pending_commands[session_key] = {
            "timestamp": now,
        ...
        # 记录命令哈希用于去重
        command_hash = self._get_request_hash("", tool_calls)
        self._recent_commands[command_hash] = now
        
        # 限制大小
        if len(self._recent_commands) > self._max_requests:
    ...
    def get_pending_command(self, request_body, user_text):
        ...
        session_key = self._get_session_key(request_body)
        pending = self._pending_commands.get(session_key)
        
        if not pending:
            return None
        
        # 检查是否过期
        if time.time() - pending["timestamp"] > self._dedup_window:
            del self._pending_commands[session_key]
            return None
    ...
    def is_command_recently_executed(self, tool_calls):
        """
        检查命令是否最近已执行过
        防止短时间内重复执行相同的工具调用
        """
        ...
        if command_hash in self._recent_commands:
            last_time = self._recent_commands[command_hash]
            # 如果在去重窗口内（30秒），视为重复
...
def build_response(request_body, is_stream=False):
    ...
    # ===== 第一级: 检查是否为重复请求（基于用户文本） =====
    if request_deduplicator.check_and_record_request(user_text):
        # 重复请求，直接返回，不做任何处理
        match_detail = {
            ...
            "tool_calls": [],
            "fallback_text": "请求已处理",
            "fallback_reason": "重复请求，已跳过",
            "is_duplicate_request": True,
        }
        result = {
            "tool_calls": None,
            "fallback_text": "",  # 返回空文本，不让奔奔助手有反馈
            "match_detail": match_detail,
        }
    else:
        # ===== 第二级: 检查是否为确认回复 =====
        pending_result = request_deduplicator.get_pending_command(request_body, user_text)
        ...
        if pending_result == "cancelled":
            ...
            # 用户确认执行之前的命令
            # 检查该命令是否最近已执行过
            if request_deduplicator.is_command_recently_executed(pending_result):
                # 命令已执行过，不再重复执行
                match_detail = {
                    "input_text": user_text,
                    "matched": True,
                    ...
                    "tool_calls": [],
                    "fallback_text": "命令已执行",
                    "fallback_reason": "命令已执行，不重复执行",
                    "is_duplicate_command": True,
                }
                result = {
                    "tool_calls": None,
                    "fallback_text": "命令已执行。",
                ...
                # 记录命令已执行
                request_deduplicator.record_command(request_body, pending_result)
        else:
            # ===== 第三级: 正常处理请求 =====
            ...
            if tool_calls:
                # 检查该命令是否最近已执行过
                if request_deduplicator.is_command_recently_executed(tool_calls):
                    # 命令已执行过，不再重复执行
                    match_detail["matched"] = True
                    match_detail["match_result"] = "text"
                    match_detail["fallback_text"] = "命令已收到"
                    match_detail["is_duplicate_command"] = True
                    result = {
                        "tool_calls": None,
                        "fallback_text": "命令已收到。",
                        "match_detail": match_detail,
                    ...
                    # 记录新的工具调用
                    request_deduplicator.record_command(request_body, tool_calls)
