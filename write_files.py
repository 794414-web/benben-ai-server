#!/usr/bin/env python3
"""Create shell scripts with proper content"""

import os

base = r'C:\Users\Administrator\Desktop\ai\rpm_package'

# build_rpm.sh
build_sh = """#!/bin/bash
# BenBen AI Assistant - RPM Build Script
set -e

BUILD_ROOT="$(pwd)/RPMBUILD"
SPEC_FILE="$(pwd)/benben-ai.spec"
TARBALL="$(pwd)/benben-ai-1.0.0.tar.gz"

echo "=== BenBen AI RPM Builder ==="
echo ""

mkdir -p "$BUILD_ROOT"/{BUILD,RPMS,SOURCES,SPECS,SRPMS}

cp "$SPEC_FILE" "$BUILD_ROOT/SPECS/"
cp "$TARBALL" "$BUILD_ROOT/SOURCES/"

echo "Building RPM..."
rpm -ba --define "_topdir $BUILD_ROOT" "$BUILD_ROOT/SPECS/benben-ai.spec"

echo ""
echo "=== Build Complete ==="
find "$BUILD_ROOT/RPMS" -name "*.rpm" -exec ls -lh {} \\;
echo ""
echo "Install: rpm -ivh $BUILD_ROOT/RPMS/noarch/benben-ai-1.0.0-1.noarch.rpm"
echo "Start:   systemctl start benben-ai"
echo "Enable:  systemctl enable benben-ai"
"""
with open(os.path.join(base, 'build_rpm.sh'), 'w') as f:
    f.write(build_sh)
print('Created build_rpm.sh')

# install.sh
install_sh = """#!/bin/bash
# BenBen AI Assistant - One-Click Install
# Usage: curl -sSL https://raw.githubusercontent.com/USER/REPO/main/install.sh | bash
set -e

echo "=== BenBen AI Installer ==="
echo ""

if [ "$EUID" -ne 0 ]; then
    echo "ERROR: Run as root"
    exit 1
fi

if [ ! -f /etc/redhat-release ]; then
    echo "ERROR: CentOS/RHEL only"
    exit 1
fi

echo "[1/4] Installing dependencies..."
yum install -y epel-release
yum install -y python3 python3-pip rpm-build tar gzip || \
    dnf install -y python3 python3-pip rpm-build tar gzip

echo "[2/4] Downloading..."
REPO_URL="https://raw.githubusercontent.com/USER/REPO/main/rpm_package"
cd /opt
rm -rf benben-build
mkdir benben-build
cd benben-build
curl -sSL "$REPO_URL/benben-ai-1.0.0.tar.gz" -o src.tar.gz
curl -sSL "$REPO_URL/benben-ai.spec" -o benben-ai.spec

echo "[3/4] Building RPM..."
mkdir -p /tmp/rpmbuild/{BUILD,RPMS,SOURCES,SPECS}
cp benben-ai.spec /tmp/rpmbuild/SPECS/
cp src.tar.gz /tmp/rpmbuild/SOURCES/
rpm -ba --define "_topdir /tmp/rpmbuild" /tmp/rpmbuild/SPECS/benben-ai.spec
RPM=/tmp/rpmbuild/RPMS/noarch/benben-ai-1.0.0-1.noarch.rpm

if [ ! -f "$RPM" ]; then
    echo "ERROR: Build failed"
    exit 1
fi

echo "[4/4] Installing..."
rpm -ivh "$RPM"

systemctl daemon-reload
systemctl enable benben-ai
systemctl start benben-ai

rm -rf /opt/benben-build /tmp/rpmbuild

echo ""
echo "=== Installation Complete ==="
echo ""
IP=$(hostname -I | awk '{print $1}')
echo "Service URL: http://$IP:9998/compatible-mode/v1"
echo "Admin Panel: http://$IP:9998/admin"
echo "Default Password: admin123456"
echo ""
echo "Commands:"
echo "  systemctl status benben-ai"
echo "  journalctl -u benben-ai -f"
"""
with open(os.path.join(base, 'install.sh'), 'w') as f:
    f.write(install_sh)
print('Created install.sh')

print()
print('All files created!')
"""
with open(os.path.join(base, 'write_files.py'), 'w') as f:
    f.write(write_files_py)
print('Created write_files.py')
